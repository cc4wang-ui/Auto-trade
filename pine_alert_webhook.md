...
### Step 2.05：v10.1 entry signal 多帶 regime + HY（選用）

如果你已套 v10.1，**進場訊號**也應該帶 regime 資訊（GAS 會在 Telegram 訊息加一行「Regime: 🟠 WARNING ↳ HY 信用壓力 (3.62%)」）。

在 Step 2 的 `alertMsg` JSON 拼接尾巴（`'"target_r":' + ...` 之後、`'"timestamp":' + ...` 之前）加：
...
長/空兩段都加（同樣字串拼接）。沒有 v10.1 補強就**不要**加（GAS 會嫌字串為空但不報錯，浪費 alert payload 字元）。
...
#### v10.1 補強：snapshot 多帶 regime + HY credit stress

如果你已套 v10.1（信用壓力子模組）→ 把 snapshot JSON 的尾巴改成：

```pine
    // v10.1 fields（無此補強就不要加，GAS 會自動跳過渲染）
    string snapRegime       = na(regime) ? "" : regime
    string snapRegimeBase   = na(regime_base) ? "" : regime_base
    string snapUpgradeReason = na(regime_upgrade_reason) ? "" : regime_upgrade_reason
    string snapHyLevel      = na(hy_pressure_level) ? "" : hy_pressure_level
    float  snapHyJump       = na(hy_weekly_jump) ? 0.0 : hy_weekly_jump
    bool   snapHyAcute      = na(hy_acute_event) ? false : hy_acute_event
    ...
    string snapMsg = '{"secret":"' + pineSecret + '",' +
       ...
       '"atr":' + str.tostring(snapAtr, "#.##") + ',' +
       '"regime":"' + snapRegime + '",' +
       '"regime_base":"' + snapRegimeBase + '",' +
       '"regime_upgrade_reason":"' + snapUpgradeReason + '",' +
       '"hy_pressure_level":"' + snapHyLevel + '",' +
       '"hy_weekly_jump":' + str.tostring(snapHyJump, "#.##") + ',' +
       '"hy_acute_event":' + (snapHyAcute ? "true" : "false") + ',' +
       '"timestamp":"' + str.tostring(time, "#") + '"}'
    alert(snapMsg, alert.freq_once_per_bar_close)
```

GAS 端：v10_state sheet 自動多 6 個 column（regime / regime_base / regime_upgrade_reason / hy_pressure_level / hy_weekly_jump / hy_acute_event）。Macro routine 撈 `read_v10_state` 時會拿到這些值，渲染到 Telegram【信用壓力】section。

⚠ 字串欄位的 escape：`regime_upgrade_reason` 含中文逗號「，」沒事，但若有 `"` 或 `\` 會破 JSON。v10.1 的 reason 由 `str.tostring(...)` 拼接，安全。
...
| `regime` / `regime_base` / `regime_upgrade_reason` | ⚠ | v10.1 信用壓力子模組；沒帶就不渲染 Regime 行 |
| `hy_pressure_level` / `hy_weekly_jump` / `hy_acute_event` | ⚠ | v10.1 HY 信用上下文；存到 v10_state sheet 給 macro routine 撈 |
...
| `regime` / `regime_base` / `regime_upgrade_reason` | ⚠ | v10.1；空字串時 GAS 寫入空 cell，read_v10_state 回 null |
| `hy_pressure_level` / `hy_weekly_jump` / `hy_acute_event` | ⚠ | v10.1；macro routine 拉這些值補【信用壓力】section |
...
> v10.0 Pine 不送 v10.1 fields → 對應 6 個欄位回 `null`，routine 自動 fallback 到 FRED 自查。
...
- `regime != null and regime != "NORMAL"` → 把 regime 帶進 `credit_stress.regime_force` + analyst_report.credit_pressure narrative
